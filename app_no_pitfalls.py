...
def _concat_same_schema(df_left: Optional[pd.DataFrame], df_right: pd.DataFrame, label_left: str, label_right: str) -> Tuple[Optional[pd.DataFrame], Optional[str]]:
    if df_right is None:
        return df_left, None
    if df_left is None:
        return df_right.copy(), None

    left_cols = [str(c) for c in df_left.columns.tolist()]
    right_cols = [str(c) for c in df_right.columns.tolist()]
    if left_cols != right_cols:
        return df_left, f"列结构不一致：{label_right} 未并入 {label_left}"

    out = pd.concat([df_left, df_right], ignore_index=True)
    return out, None


def _prepare_uploaded_files(files: List) -> Tuple[List[dict], List[str], List[dict]]:
    prepared: List[dict] = []
    warnings: List[str] = []
    ...
    for f in files:
        ...
        if store_id is None:
            store_id = "UNKNOWN"
        meta = {"name": name, "bytes": b, "kind": kind, "store_id": store_id}
        prepared.append(meta)
        groups.setdefault((store_id, kind), []).append(meta)

    for (store_id, kind), items in sorted(groups.items(), key=lambda x: (x[0][0], x[0][1])):
        if kind in {"daily", "dish", "pay"} and len(items) > 1:
            merge_logs.append({
                "store_id": store_id,
                "kind": kind,
                "count": len(items),
            ...
            for item in items:
                warnings.append(f"无法识别文件类型：{item['name']}（已跳过）")

    return prepared, warnings, merge_logs


def parse_uploaded(files: List[dict], rule_file) -> Tuple[List[StoreBundle], List[str], List[Rule]]:
    rules: List[Rule] = []
    ...
    for f in files:
        ...
        b = f["bytes"]
        kind = f.get("kind")
        store_id = f.get("store_id") or "UNKNOWN"

        if kind == "unknown":
            continue

        bundle = upsert(store_id)

        if kind == "daily":
            df, _ = read_any_table(b, name, DAILY_MUST)
            merged, warn = _concat_same_schema(bundle.daily, df, f"门店 {store_id} 的日销售数据", name)
            bundle.daily = merged
            if warn:
                warnings.append(warn)
        elif kind == "dish":
            df, _ = read_any_table(b, name, DISH_MUST)
            merged, warn = _concat_same_schema(bundle.dish, df, f"门店 {store_id} 的菜品明细", name)
            bundle.dish = merged
            if warn:
                warnings.append(warn)
        elif kind == "pay":
            df, _ = read_any_table(b, name, PAY_MUST)
            merged, warn = _concat_same_schema(bundle.pay, df, f"门店 {store_id} 的支付明细", name)
            bundle.pay = merged
            if warn:
                warnings.append(warn)
        else:
...
def main() -> None:
    ...
    prepared_files, pre_warnings, merge_logs = _prepare_uploaded_files(files)
    if merge_logs:
        with st.expander("✅ 自动合并日志", expanded=False):
            log_df = pd.DataFrame([
                {
                    "门店": x["store_id"],
                    "类型": x["kind"],
    ...
    bundles, warnings, rules = parse_uploaded(prepared_files, rule_file)
